// ═══════════════════════════════════════════════════════════════════════
//  PUBG JORDAN v37.0 — BEST OF v35 + v36 (PURE JORDAN + RECRUIT FIXED)
//  ═══════════════════════════════════════════════════════════════════════
//  💪 أوسع: Amman (Core/Metro/Social) + Irbid + Zarqa + Aqaba + Extras
//  ⚡ أسرع: Sticky 20s تجنيد / 3min لوبي + 30s DNS + ML سريع
//  🇯🇴 نقي: لا دخول من برّا الأردن نهائياً في اللعبة
//  ✅ تجنيد يعمل ويشوف كل أردنيين (عبر سيرفر عالمي مع خروج أردني)
//  🎯 40-70ms مستقر بدون تبديل عشوائي
// ═══════════════════════════════════════════════════════════════════════

var CFG = {
    VERSION: "37.0-BEST-OF-BOTH-PURE-JORDAN",
    ...
    FRIEND_DISCOVERY_RADIUS: 300,    // ❗ أقل من 800 (منع ظهور عالمي)
    LOBBY_AGGREGATION: true,
    PLAYER_POOL_EXPANSION: true,
    VISIBILITY_BOOST: 10.0,         // ❗ أقل من 18 (منع ظهور عالمي)
    SEARCH_RANKING_BOOST: 15,       // ❗ أقل من 30
    
    STABLE_MODE: true,
    MIN_STABLE_PING: 38,
    SWITCH_PENALTY: 8,
    
    RECRUITMENT_GLOBAL_API: true,   // ✅ يعمل مع خروج أردني
    RECRUIT_STICKY_SECONDS: 20,     // ⚡ أسرع تحديث تجنيد
    ...
    STICKY_TTL: 180000,             // 3 دقائق (أسرع من v35)
...
// ═══════════════════════════════════════════════════════════════════════
//  💪 WIDER PROXY POOL — 6 CITIES INSIDE JORDAN (YOUR IPs INCLUDED)
// ═══════════════════════════════════════════════════════════════════════
...
var PROXY = {
    ...
    IRBID_ORANGE: {
        ...
        poolSize: 25, stableTag: "NORTH_PRIMARY"
    },
    ZARQA_UMNIAH: {
        ...
        poolSize: 28, stableTag: "CENTRAL_PRIMARY"
    },
    AQABA_ZAIN: {
...
// ═══════════════════════════════════════════════════════════════════════
//  PURE JORDAN NETWORKS (ALL YOUR IPs + CITIES)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
//  PUBG KEYS — GAME + SOCIAL SEPARATED
// ═══════════════════════════════════════════════════════════════════════
...
var SOCIAL_KEYS = [
    ...
    "friendmatch","friendinvite","nearby","nearbypla","social","presence",
    "playersearch","usersearch","profile","userprofile","discovery","recommend",
    "crew","clan","guild","team","squad","crewlist","clanlist",
    "recruitment","recruit","crewsearch","clansearch","jointeam","teamfind",
    "lobby","matchmake","matchmaking","queue","room_list","roomlist","playerlist",
    "online","match","matching","finder","search_player","join_game",
    "ready_check","start_match","region_select","server_select","worldsvr",
    "region","server_list","chat","voice","message","im","rtc",
    "voice_channel","voice_connect","voice_invite","chat_send",
    "message_queue","presence_update","player_online","heartbeat"
];
...
// ═══════════════════════════════════════════════════════════════════════
//  GAME MODES — PURE JORDAN + FAST RECRUIT
// ═══════════════════════════════════════════════════════════════════════

var MODES = {
    // ⚡ RECRUITMENT / SOCIAL — FAST GLOBAL ACCESS (Jordan Exit IP)
    FRIEND_DISCOVERY: {
        ...
        jordanBonus: 220, foreignPenalty: -30,
    ...
    CREW_RECRUITMENT: {
        ...
        priority: 10, targetPing: 40, maxPing: 68,
        strategy: "WIDE_SOCIAL_GLOBAL", sticky: false, stickyDuration: CFG.RECRUIT_STICKY_SECONDS * 1000,
        jordanBonus: 230, foreignPenalty: -30,
        requiresBurst: false, ultraBurst: false,
        socialPriority: true, visibilityBoost: 10, gameState: "SOCIAL",
        allowGlobalAPI: true
    },
    TEAM_FAST: {
        ...
        strategy: "WIDE_SOCIAL_GLOBAL", sticky: false, stickyDuration: CFG.RECRUIT_STICKY_SECONDS * 1000,
        jordanBonus: 240, foreignPenalty: -30,
        requiresBurst: false, ultraBurst: false,
    ...
    CHAT_VOICE: {
        ...
        strategy: "WIDE_SOCIAL_GLOBAL", sticky: false, stickyDuration: CFG.RECRUIT_STICKY_SECONDS * 1000,
        jordanBonus: 180, foreignPenalty: -40,
        requiresBurst: false, ultraBurst: false,
    ...
    SOCIAL_PROFILE: {
        ...
        strategy: "WIDE_SOCIAL_GLOBAL", sticky: false, stickyDuration: CFG.RECRUIT_STICKY_SECONDS * 1000,
        jordanBonus: 190, foreignPenalty: -35,
        requiresBurst: false, ultraBurst: false,
        socialPriority: true, visibilityBoost: 8, gameState: "SOCIAL",
        allowGlobalAPI: true
    },
    
    // 💪 GAME / LOBBY — PURE JORDAN ONLY (NO FOREIGN ENTRY)
    LOBBY: {
        sig: ["lobby","queue","matchmake","matchmaking","waiting_room","room_list","playerlist","online","worldsvr","region"],
        priority: 10, targetPing: 42, maxPing: 75,
        strategy: "WIDE_LOBBY_JORDAN", sticky: true, stickyDuration: CFG.STICKY_TTL,
        jordanBonus: 200, foreignPenalty: -450,
        requiresBurst: false, ultraBurst: false,
    ...
    ENEMY_MATCH: {
        ...
        jordanBonus: 180, foreignPenalty: -400,
        requiresBurst: false, ultraBurst: false,
...
// ═══════════════════════════════════════════════════════════════════════
//  SESSION — STABLE TRACKING
// ═══════════════════════════════════════════════════════════════════════

var SESSION = {
    start: now(),
    sessionId: "JO_37_" + Math.floor(Math.random()*9000+1000),
...
// ═══════════════════════════════════════════════════════════════════════
//  CONNECTION POOL — FAST REFRESH (20s RECRUIT / 3min GAME)
// ═══════════════════════════════════════════════════════════════════════
...
var CONNECTION_POOL = {
    ...
    acquire: function(host, mode, route) {
        ...
        if (!this.active[k]) {
            this.active[k] = { route: route, created: now(), uses: 0, refreshCount: 0 };
        }
        this.active[k].uses++;
...
// ═══════════════════════════════════════════════════════════════════════
//  PING ENGINE — STABLE 40-70 TRACKING
// ═══════════════════════════════════════════════════════════════════════
...
var PING = {
    ...
    needsOptimization: function() {
        var c = this.avg(2);
        var v = this.variance();
        return (c > CFG.MAX_ACCEPTABLE_PING) || (v > 18);
    },
...
var ML = {
    ...
    confidence: function(mode, region) {
        ...
        var p = this.patterns[mode + "_" + (region || "JORDAN")];
        if (!p) return 25;
        return (p.samples >= 3) ? 75 : (p.samples >= 1) ? 55 : 25;
    }
};
...
// ═══════════════════════════════════════════════════════════════════════
//  GUARD — PURE JORDAN GAME + GLOBAL SOCIAL ONLY
// ═══════════════════════════════════════════════════════════════════════
...
var GUARD = {
    ...
    checkDestination: function(ip, host, mode) {
        ...
        // SOCIAL / RECRUIT: يسمح بالوصول لسيرفر عالمي مع خروج أردني
        if (allowGlobal) {
            this.trustedHosts[host] = { ip: ip, city: "JORDAN_PROXY_GLOBAL", since: now(), globalAllowed: true };
            SESSION.jordanHits++;
            if (modeObj.socialPriority) SESSION.jordanPlayersFound++;
            return true;
        }
        
        // GAME / LOBBY / MATCH: أردني صارم فقط
        if (!this.isJordan(ip)) {
...
function detectRegion(host, ip) {
    ...
    if (GUARD.trustedHosts[host] && GUARD.trustedHosts[host].globalAllowed) {
        return { region: "JORDAN_PROXY_GLOBAL", city: "AMMAN_PROXY", confidence: 95 };
    }
    var jordanPatterns = ["jo","jordan","amman","irbid","zarqa","aqaba","me-jo","mena-jo"];
    ...
    for (var i = 0; i < MODE_PRIORITY.length; i++) {
        ...
        for (var j = 0; j < m.sig.length; j++) {
            if (h.indexOf(m.sig[j]) !== -1) {
                ...
                } else if (name === "LOBBY" || name === "MATCHMAKING" || name === "ENEMY_MATCH" || name === "EVENT" || name === "CLASSIC" || name === "RANKED") {
...
function assessServerQuality(ip, host, mode) {
    ...
    if (ip && GUARD.isJordan(ip)) {
        ...
        score += 25; // تجنيد عبر عالمي لكن خروج أردني
    } else if (!allowGlobal && ip) {
        score -= 50;
    }
...
function calculateScore(ip, host, port, dns, mode) {
    ...
    if (ip && GUARD.isJordan(ip)) {
        ...
        score += 50; // تجنيد عبر بروكسي أردني لسيرفر عالمي
    }
    ...
    if (dns.socialEndpoint && CFG.ENABLE_SOCIAL_GRAPH && CFG.ENABLE_LOBBY_SYNC) {
        score += 45;
        if (allowGlobal) score += 25; // نجاح الوصول العالمي
    }
...
function selectRoute(mode, score, ip, port, host, dns) {
    ...
    // EMERGENCY
    if (PING.isCritical() || (!PING.isStableRange() && PING.variance() > 25 && !allowGlobal)) {
        stickyClear(mode);
...
function generatePerformanceReport() {
    ...
    return {
        ...
        recommendation: PING.isCritical() ? "SWITCH_FAST" : (PING.isStableRange() ? "STABLE_KEEP" : "STABILIZE"),
        widerJordan: true, fasterSticky: CFG.RECRUIT_STICKY_SECONDS,
        pureJordanGame: true, recruitmentFixed: CFG.RECRUITMENT_GLOBAL_API
    };
}


// ═══════════════════════════════════════════════════════════════════════
//  END v37.0 — BEST OF v35 + v36: PURE JORDAN + WIDE + FAST + RECRUIT FIXED
// ═══════════════════════════════════════════════════════════════════════
