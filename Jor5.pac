function FindProxyForURL(url, host) {
  ...
  var DNS_CACHE_TTL_MAX_MS  = 300000;
  var PROBE_COUNT           = 3;
  var EWMA_ALPHA            = 0.3;
  ...
  for (var i = 0; i < PROXIES.length; i++) {
    var p = PROXIES[i];
    if (!__JO_ADV.proxies[p.ip]) {
      __JO_ADV.proxies[p.ip] = {
        ewma: null,
        variance: null,
        lastProbe: 0,
        failures: 0,
  ...
  function cacheDNS(name) {
    var e = __JO_ADV.dns[name];
    var n = nowMs();
    if (e && (n - e.t) < (e.ttl || DNS_CACHE_TTL_BASE_MS)) return e.ip;
    var ip = dnsResolve(name);
    if (ip) {
      __JO_ADV.dns[name] = { ip: ip, t: n, ttl: DNS_CACHE_TTL_BASE_MS };
      return ip;
    } else {
      __JO_ADV.dns[name] = { ip: null, t: n, ttl: DNS_CACHE_TTL_MIN_MS };
      return null;
    }
  }
  ...
  function isYouTubeHost(h) {
    ...
    if (shExpMatch(x, "*.yt3.ggpht.com")) return true;
    if (shExpMatch(x, "*.youtubei.googleapis.com")) return true;
  ...
  function multiProbe(target, n) {
    ...
    var vsum = 0;
    for (var s = 0; s < samples.length; s++) vsum += Math.pow(samples[s] - mean, 2);
    var variance = vsum / samples.length;
    return { ip: ip, mean: mean, variance: variance, samples: samples };
  }

  function updateProxyStats(proxyIp, sampleMean, sampleVar) {
    var st = __JO_ADV.proxies[proxyIp];
    if (!st) return;
    if (st.ewma === null) st.ewma = sampleMean;
    else st.ewma = (1 - EWMA_ALPHA) * st.ewma + EWMA_ALPHA * sampleMean;
    if (st.variance === null) st.variance = sampleVar;
    else st.variance = (1 - VARIANCE_ALPHA) * st.variance + VARIANCE_ALPHA * sampleVar;
    st.lastProbe = nowMs();
  }

  function proxyBackoff(proxyIp) {
    var st = __JO_ADV.proxies[proxyIp];
    if (!st) return;
    st.failures = (st.failures || 0) + 1;
    var backoff = Math.min(MAX_FAILURE_BACKOFF, FAILURE_BACKOFF_BASE * Math.pow(2, st.failures - 1));
    st.backoffUntil = nowMs() + backoff;
  }

  function isProxyAvailable(proxyIp) {
    var st = __JO_ADV.proxies[proxyIp];
    if (!st) return true;
    return (st.backoffUntil || 0) < nowMs();
  }

  function hashHostToIndex(host, n) {
    var h = 0;
    for (var i = 0; i < host.length; i++) h = ((h << 5) - h) + host.charCodeAt(i);
    return Math.abs(h) % n;
  }

  function getStickyPort(host, proxy) {
    var key = host + "@" + proxy.ip;
    var rec = __JO_ADV.stickyPorts[key];
    var n = nowMs();
    if (rec && (n - rec.t) < STICKY_PORT_TTL) return rec.port;
    ...
    var port = ports[idx];
    __JO_ADV.stickyPorts[key] = { port: port, t: n, tried: [port] };
    return port;
  }

  function evaluateProxy(proxy, host) {
    if (!isProxyAvailable(proxy.ip)) return { ok: false, score: -99999 };
    var probe = multiProbe(proxy.ip, PROBE_COUNT);
    if (!probe.ip) { proxyBackoff(proxy.ip); return { ok: false, score: -99999 }; }
    updateProxyStats(proxy.ip, probe.mean, probe.variance);
    var st = __JO_ADV.proxies[proxy.ip];
    var mean = st.ewma || probe.mean;
    var variance = st.variance || probe.variance;
    var varianceFactor = 1 + Math.sqrt(variance) / (1 + mean);
    var score = proxy.weight / (1 + mean) / varianceFactor;
    if (proxy.ports && proxy.ports.indexOf(20001) !== -1) score *= 1.12;
    return { ok: true, score: score, mean: mean, variance: variance, probe: probe };
  }
  ...
  function chooseBestProxyForHost(host) {
    ...
    for (var i = 0; i < PROXIES.length; i++) {
      var ev = evaluateProxy(PROXIES[i], host);
      if (ev.ok && ev.score > bestScore) {
        bestScore = ev.score;
        best = { proxy: PROXIES[i], eval: ev };
      }
    }
    return best;
  }

  function decideProxySwitch(host, candidate) {
    var session = __JO_ADV.sessions[host];
    if (!session || !session.proxyIp) {
      __JO_ADV.sessions[host] = { proxyIp: candidate.proxy.ip, t: nowMs(), backoffUntil: 0 };
      return candidate.proxy;
    }
    if (session.proxyIp === candidate.proxy.ip) {
      session.t = nowMs();
      return candidate.proxy;
    }
    var curr = __JO_ADV.proxies[session.proxyIp];
    var currScore = curr && curr.ewma ? (1 / (1 + curr.ewma)) : 0.0001;
    var candScore = candidate.eval.score || 0;
    if (candScore > currScore * SWITCH_THRESHOLD) {
      __JO_ADV.sessions[host] = { proxyIp: candidate.proxy.ip, t: nowMs(), backoffUntil: 0 };
      return candidate.proxy;
    }
    for (var i = 0; i < PROXIES.length; i++) {
      if (PROXIES[i].ip === session.proxyIp) return PROXIES[i];
    }
    __JO_ADV.sessions[host] = { proxyIp: candidate.proxy.ip, t: nowMs(), backoffUntil: 0 };
    return candidate.proxy;
  }
  ...
  if (!candidate) {
    var fb = PROXIES[0];
    var fbPort = getStickyPort(h, fb);
    if (!FORBID_DIRECT_GLOBAL && hostIsJordanIP) return "DIRECT";
    return "PROXY " + fb.ip + ":" + fbPort;
  }

  var chosen = decideProxySwitch(h, candidate);

  if (isGame) {
    var gp = getStickyPort(h, chosen);
    return "SOCKS5 " + chosen.ip + ":" + gp;
  }

  if (isLobby) {
    var lp = getStickyPort(h, chosen);
