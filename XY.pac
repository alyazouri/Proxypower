function FindProxyForURL(url, host) {
  ...
  var PORTS = {
    LOBBY: [443, 8443, 808],
    MATCH: [10012, 10013], // منافذ اللعب داخل المباراة
    RECRUIT_SEARCH: [10010, 10012, 10013, 10014, 10015, 10016, 10017, 10018, 10019, 10020],
    UPDATES: [80, 443, 8443, 808],
    CDNs: [80, 443, 808]
  };

  // أوزان اختيار المنفذ (عشان نثبت على الأفضل)
  var PORT_WEIGHTS = {
    LOBBY: [5, 3, 2],
    MATCH: [3, 2], // أولوية للـ10012
    RECRUIT_SEARCH: [4, 3, 3, 2, 2, 2, 2, 2, 2, 1],
    UPDATES: [5, 3, 2, 1],
    CDNs: [3, 2, 2]
  };

  // جميع الرينجات الأردنية بدون تكرار
  // مرتبة من الأوسع إلى الأضيق (أكبر مساحة IP أولاً)
  var JO_IP_RANGES = [
    ["91.176.0.0",  "91.184.255.255"], // أوسع
    ...
    ["217.178.0.1", "217.178.255.255"] // أضيق
  ];

  // سياسة الإلزام: كل الفئات الحساسة لازم تكون أردنية
  var STRICT_JO_FOR = {
    LOBBY: true,
    MATCH: true,
    RECRUIT_SEARCH: true,
  ...
  // لو السيرفر مش أردني → بلوك
  var FORBID_NON_JO = true;
  var BLOCK_REPLY = "PROXY 0.0.0.0:0";

  // ثبات اختيار المنفذ لمدة ساعة (عشان البنق ما يقفز)
  var STICKY_SALT = "JO_STICKY";
  var STICKY_TTL_MINUTES = 60;
  var JITTER_WINDOW = 0;

  // حل DNS داخل cache لمدة 15 ثانية فقط
  ...
  // هل الـIP داخل أي نطاق أردني؟
  // preferPriority = true يعطي أولوية لنطاق خاص مفضل بالمباريات
  function ipInAnyJordanRange(ip, preferPriority) {
    if (!ip) return false;
    var ipNum = ipToInt(ip);

    if (preferPriority) {
      // نطاق أولوية عالي للمباريات
      var preferredRange = ["91.106.96.0", "91.106.111.255"];
      var pStart = ipToInt(preferredRange[0]);
      var pEnd   = ipToInt(preferredRange[1]);
      if (ipNum >= pStart && ipNum <= pEnd) return true;
    }
  ...
  // مقارنة الدومين ضد الباترنات (wildcards)
  function hostMatchesAnyDomain(h, patterns) {
    for (var i = 0; i < patterns.length; i++) {
      if (shExpMatch(h, patterns[i])) return true;

      var suffix = patterns[i].replace(/^\*\./, ".");
      if (h.slice(-suffix.length) === suffix) return true;
    }
    return false;
  }

  // مقارنة الـURL نفسه مع الباترنات
  ...
  // Sticky port per category (عشان البنج يضل ثابت وما يبدل بورت كل شوي)
  function proxyForCategory(category) {
    var key = STICKY_SALT + "_PORT_" + category;
    var ttlMs = STICKY_TTL_MINUTES * 60 * 1000;
    ...
    var chosen = weightedPick(PORTS[category], PORT_WEIGHTS[category]);
    CACHE._PORT_STICKY[key] = { p: chosen, t: now };
    return "PROXY " + PROXY_HOST + ":" + chosen;
  }

  // DNS resolve مع كاش داخلي
  function resolveDstCached(h, ttl) {
    if (!h) return "";

    // إذا host أصلاً IP جاهز
    if (/^\d+\.\d+\.\d+\.\d+$/.test(h)) return h;

    var cached = CACHE.DST_RESOLVE_CACHE[h];
  ...
  // ---------------- فحص أن العميل والبروكسي أردنيين ----------------

  var geoTTLms = STICKY_TTL_MINUTES * 60 * 1000;
  var clientKey = STICKY_SALT + "_CLIENT_JO";
  var clientOK;
  var cCached = CACHE[clientKey];

  if (cCached && (now - cCached.t) < geoTTLms) {
    clientOK = cCached.ok;
  } else {
  ...
  // لو يا اللاعب أو البروكسي مش أردني -> بلوك / أو DIRECT حسب السياسة
  if (!(clientOK && proxyOK)) {
    return FORBID_NON_JO ? BLOCK_REPLY : "DIRECT";
  }

  // ---------------- قرار: أي بروكسي نستخدم و هل نسمح بالوجهة؟ ----------------

  function requireJordanDestination(category, h) {
    var dstIP = resolveDstCached(h, DST_RESOLVE_TTL_MS);
    var preferPriority = (category === "MATCH"); // نعطي MATCH أولوية أقرب
    if (!ipInAnyJordanRange(dstIP, preferPriority)) {
      return FORBID_NON_JO ? BLOCK_REPLY : "DIRECT";
    }
    return proxyForCategory(category);
  }

  // 1) لو مسار الـURL يدل على وظيفة معروفة (لوبي، ماتش، تحديث...)
  ...
  // 2) لو الـhost تبع الدومين يطابق دومينات PUBG
  ...
  // 3) لو الوجهة IP مباشر (بدون دومين) وهو أردني -> نعامله زي LOBBY
  var dstIPDirect = /^\d+\.\d+\.\d+\.\d+$/.test(host)
    ? host
    : resolveDstCached(host, DST_RESOLVE_TTL_MS);

  if (dstIPDirect && ipInAnyJordanRange(dstIPDirect, false)) {
    return proxyForCategory("LOBBY");
  }

  // أي شيء ثاني: DIRECT أو BLOCK حسب السياسة العامة
  return "DIRECT";
}
