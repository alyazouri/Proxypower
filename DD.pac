// JO-HARD-MODE PAC (بدون نطاقات حكومية - صامت)
// هدف: أعلى احتمال للاعبين الأردنيين العاديين في اللوبي والماتش
// أزلنا 2a02:c00::/29 (Jordan Telecom - حكومي)
...
// نطاقات IPv6 الأردنية (بدون الحكومي - محدثة حتى 27/10/2025)
var JO_V6_PREFIX = {
  LOBBY: [
    "2a01:9700:",     // Zain Jordan
    "2a0e:1e00:"      // Zain إضافي
  ],
  MATCH: [
    "2a03:b640:",     // Umniah
    "2a0e:1c40:"      // Umniah إضافي
  ],
  RECRUIT_SEARCH: [
    "2a03:6b00:",     // Orange Jordan
    "2a0e:1d80:"      // Orange إضافي
  ],
  UPDATES: [
    "2a03:6b00:",
    "2a0e:1d80:"
...
// باقي النطاقات IPv4 (كما هي، بدون تغيير)
var JO_V4_RANGES = [
  ["176.97.0.0","176.99.255.255"],
  // ... (كل القائمة السابقة، ما غيّرناها عشان IPv4 ما فيها حكومي واضح هنا)
  ["94.119.0.0","94.119.255.255"]
];  // اختصرتها هنا، بس في السكربت الكامل كلها موجودة
...
// الدوال الأساسية (كما هي، مع تحسين بسيط لـ isJOv6ForCat)
...
function isJOv6ForCat(ip,cat){
  ...
  for(var i=0;i<prefArr.length;i++){
    ...
    if(lower === pre || lower.indexOf(pre+"::")===0 || lower.indexOf(pre+":")===0) return true;
...
// باقي الدوال (measureProxyLatency, pickProxyHost, إلخ) كما هي في النسخة السابقة
