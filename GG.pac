/* ==== PAC: PUBG Jordan-Only IPv6 (JO-only match & lobby, sticky 5m) ==== */
/* يمرّ الاتصال فقط إذا كان ضمن نطاقات IPv6 الأردنية (للعبة). غير اللعبة: DIRECT. */

var PROXY_CANDIDATES = ["2a03:6b01:8000::2"]; // عدّلها لعناوين بروكسي IPv6 أردنية فعلية لو عندك أكثر
var FIXED_PORT = { LOBBY:443, MATCH:20001, RECRUIT_SEARCH:443, UPDATES:80, CDN:80 };

/* نطاقات IPv6 الأردنية (كما زوّدتني) */
var JO_V6_PREFIXES = [
  "2a00:18d8::/29",       // Orange
  "2a03:6b01::/34",       // Zain
...
var DNS_TTL_MS=15000, PROXY_STICKY_TTL_MS=300000 /*5m*/, GEO_TTL_MS=3600000;
...
/* ===== كاش DNS (single result؛ PAC محدود) ===== */
...
function pickProxyHost(){
  ...
  for(var i=0;i<PROXY_CANDIDATES.length;i++){
    ...
    var l=measureProxyLatency(c);
    if(l<bestLat){best=c;bestLat=l;}
  }
...
function proxyFor(cat){var h=pickProxyHost();var p=FIXED_PORT[cat]||443;return "PROXY "+bracketIfV6(h)+":"+p;}

/* ===== السماح فقط لو الوجهة IPv6 أردني ===== */
function allowJOv6Host(host){
...
function FindProxyForURL(url,host){
  ...
  // لوجي/ماتش/تجنيد/تحديثات/CDN: لازم الوجهة IPv6 أردني حصراً
  if(urlMatch(url,URL_PATTERNS.MATCH)||hostMatch(host,PUBG_DOMAINS.MATCH))
    return allowJOv6Host(host)?proxyFor("MATCH"):"PROXY 0.0.0.0:0";

  if(urlMatch(url,URL_PATTERNS.LOBBY)||hostMatch(host,PUBG_DOMAINS.LOBBY))
    return allowJOv6Host(host)?proxyFor("LOBBY"):"PROXY 0.0.0.0:0";

  if(urlMatch(url,URL_PATTERNS.RECRUIT_SEARCH)||hostMatch(host,PUBG_DOMAINS.RECRUIT_SEARCH))
    return allowJOv6Host(host)?proxyFor("RECRUIT_SEARCH"):"PROXY 0.0.0.0:0";

  if(urlMatch(url,URL_PATTERNS.UPDATES)||hostMatch(host,PUBG_DOMAINS.UPDATES))
    return allowJOv6Host(host)?proxyFor("UPDATES"):"PROXY 0.0.0.0:0";

  if(urlMatch(url,URL_PATTERNS.CDN)||hostMatch(host,PUBG_DOMAINS.CDN))
    return allowJOv6Host(host)?proxyFor("CDN"):"PROXY 0.0.0.0:0";

  // غير PUBG: خليه DIRECT عشان تصفّحك ما يتعطّل
  return "DIRECT";
}
