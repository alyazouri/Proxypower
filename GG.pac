/**** PAC: PUBG — Zain + Umniah Residential (Final v7, Media + image hosts excluded) ****/

var PROXY_HOST       = "91.106.109.12";
var FORBID_NON_JO    = true;
var TRUST_PROXY_IS_JO = false;

/* === Media / image / analytics exclusions (DIRECT) === */
var MEDIA_DOMAINS = [
  // YouTube / Google Video / Google image hosts
  ...
  // Analytics/telemetry you asked to bypass
  "browser-intake-datadoghq.com","*.browser-intake-datadoghq.com"
];

/* === Ports & weights === */
var PORTS = {
  LOBBY:[443,8443], MATCH:[20001,20003],
  RECRUIT_SEARCH:[10012,10013], UPDATES:[80,443,8443], CDNS:[80,443]
};
...
function isBypass(host){
  if(!host) return true;
  var h = host.toLowerCase();
  if (isMediaHost(h)) return true;                      // media/analytics DIRECT
...
function FindProxyForURL(url, host){
  ...
  if (!dst || !ipInJordan(dst)) return FORBID_NON_JO ? "PROXY 0.0.0.0:0" : "DIRECT";
