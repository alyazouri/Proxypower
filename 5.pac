...
{host:"109.107.240.101",region:"JO",socks:[20001,20002,20003,20004],http:[443,8080,8085,8443]},
...
if (!JO_ROUTE_ONLY) { STATE.JO_POOL = PROXY_POOL.slice(0); return }
var out = []
for (var i=0;i<PROXY_POOL.length;i++) {
var p = PROXY_POOL[i]
if (p.region === "JO") { out.push(p); continue }
if (p.ip && isInJO(p.ip)) { out.push(p); continue }
if (p.host) {
var ip = dnsCachedResolve(p.host)
if (ip && isInJO(ip)) { out.push(p) }
}
}
if (out.length === 0) {
out.push({host:"91.106.109.12",region:"JO",socks:[20001,20002,20003,20004],http:[443,8080,8085,8443]})
}
STATE.JO_POOL = out
}
...
function buildProxyString(proxyObj, port) {
if (proxyObj.ip) return "SOCKS " + proxyObj.ip + ":" + (proxyObj.port || port)
for (var i=0;i<(proxyObj.socks||[]).length;i++)
if (proxyObj.socks[i] === port) return "SOCKS " + proxyObj.host + ":" + port
return "PROXY " + proxyObj.host + ":" + port
}
...
function hostIsPreferredJO(h) {
var l = h.toLowerCase()
for (var i=0;i<PREFER_JO_HOSTS.length;i++)
if (l.indexOf(PREFER_JO_HOSTS[i]) !== -1) return true
return false
}
...
if (hostMatchesAny(host,pubg_domains) || urlPathKeywords(url) || ipInRanges(host) || hostIsPreferredJO(host)) {
var bucket = Math.floor(Date.now()/jWin)
var seed = hash32(host + "|" + bucket + "|" + q)
rotateWeightsOnQuality(q)
var selected = pickProxyFromPool(seed)
var port = pickWeightedPort(STATE.WEIGHTS, seed)
var sticky = STATE.STICKY[host]
if (sticky && (Date.now()-sticky.ts) < STICKY_TTL_MS) { selected = sticky.selected; port = sticky.port }
else { STATE.STICKY[host] = { selected:selected, port:port, ts:Date.now() } }
return buildFallbackChain(selected, port)
}
} catch(e) {
return DEFAULT_FALLBACK
}

if (FORBID_DIRECT_GLOBAL && (hostMatchesAny(host,pubg_domains) || urlPathKeywords(url) || ipInRanges(host))) return DEFAULT_FALLBACK
